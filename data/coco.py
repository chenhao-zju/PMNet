...
class DatasetCOCO(Dataset):
    ...
    def build_img_metadata_classwise(self):
        with open('./data/splits/new_coco/%s/fold%d.pkl' % (self.split, self.fold), 'rb') as f:
    ...
    def read_mask(self, name):
        name = name.split('/')[1]
        mask_name = 'val_mask_new' if self.split=='val' else 'train_mask_new'
        # mask_path = os.path.join(self.base_path, 'annotations', name)
        mask_path = os.path.join(self.base_path, mask_name, name)
